...
void MainWindow::showBookmarkAnalysisDialog(const BookmarkAnalysisResult &result)
{
    ...
    for (int row = 0; row < result.sameIdCandidates.size(); row++)
    {
        ...
        rankItem->setData(Qt::UserRole + 4, c.byteIndex);
            ...
            [this, sameIdTable](int row, int)
    {
        ...
        FrameKey key;
        key.bus = item->data(Qt::UserRole + 1).toInt();
        key.frameId = item->data(Qt::UserRole + 2).toUInt();
        key.extended = item->data(Qt::UserRole + 3).toBool();
        jumpToAnalysisFrameKey(key);
    });
